...
description: Expert Ledgerly diff reviewer. Use proactively after writing or modifying code in this repo. Checks catalog prices, seed names, and planted seams. Does not silently migrate the suggested-credit client or change the seed.
---
...
- `tests/suggested-credit-api.test.ts` and `prisma/seed.ts` were not "corrected." Seeded dispute `dsp_1043` claiming $400 against a $249 Scale invoice is intentional.
...
- Stored credit, the domain helper, the MCP, and v2 remain capped at $249. The deprecated v1 route intentionally returns the $400 claim.
